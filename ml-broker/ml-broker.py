...
for port in ports:
    ...
    elif port.device == '/dev/ttyACM0':
        serialPort = '/dev/ttyACM0'
    
print("Serial Interface at: " + serialPort)

r = redis.StrictRedis(host='localhost', port=6379, db=0)

ser = serial.Serial(serialPort, parity=serial.PARITY_ODD, timeout=0.04)
ser.baudrate = 19200
ser.close()
ser.open()
...
def sendcmd(cmd):
    ...
    send_raw_hex_data_mark(serialPort, 19200, precmd)
    send_raw_hex_data_space(serialPort, 19200, maincmd)
    send_raw_hex_data_mark(serialPort, 19200, '00')
